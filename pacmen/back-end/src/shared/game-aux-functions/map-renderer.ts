import {
  ...
  export function printMapLayout(map: GameMap, players: Player[]) {
    let layout = '';
      ...
      for (let j = 0; j < map.cols; j++) {
        ...
        if (cell?.playerIds?.length > 0) {
          const player: Player = players.find(
            (player) => player.id === cell.playerIds[0],
          );
  ...
  export function printByPlayerState(row: string, state: PlayerState, pacman:boolean): string {
    switch (state) {
  ...
  export function printByNPCName(row: string, name: string): string {
    if (!npcSelector[name]) {
      return row;
    }
    switch (name) {
  ...
  export function printByGameItem(row: string, item: GameItem): string {
    switch (item) {
  ...
  export function printByCellType(row: string, type: CellType): string {
    switch (type) {
      case CellType.WALL:
        row += `⬜`;
        break;
