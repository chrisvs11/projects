...
export const SpriteAnimator: FC<SpriteAnimatorProps> = ({
  ...
  useEffect(() => {
    ...
    (sprite.src = src),
      (sprite.onload = () => {
        ctx?.clearRect(0, 0, frameWidth * scale, frameWidth * scale);
        ctx!.imageSmoothingEnabled = false
        ctx?.drawImage(
          sprite,
          spriteDirection === "horizontal"
            ? offsetX + currentFrame * frameWidth
            : offsetX,
          spriteDirection === "vertical"
            ? offsetY + currentFrame * frameWidth
            : offsetY,
          frameWidth,
          frameWidth,
          0,
  ...
  return (
    <div>
      <canvas ref={canvasRef} width={frameWidth*scale} height={frameWidth*scale}></canvas>
    </div>
  );
};
